...
tanggalInput.addEventListener("input", function () {
    ...
    if(value){
        ...
        minimalTanggal.setDate(minimalTanggal.getDate() + 7);

        if(tanggalAcara < minimalTanggal){
            tanggalValid = false;
            errorTanggal.style.display = "block";
            errorTanggal.innerHTML = `<i class="fas fa-circle-exclamation me-1"></i> Minimal H-7 sebelum acara.`;
...
// ==========================================
// 2. FUNGSI UI (NOMINAL & PREVIEW)
// ==========================================
function updateNominal(){
    const isLunas = document.getElementById('lunas').checked;
    const total = parseInt(document.getElementById('total_harga').value);
    const nominal = isLunas ? total : total * 0.5;
    document.getElementById('labelPembayaran').innerText = isLunas ? "Nominal Lunas:" : "Nominal DP:";
    document.getElementById('nominalPembayaran').innerText = "Rp" + nominal.toLocaleString('id-ID');
}
...
function updateInstruction() {
    ...
    document.getElementById('instruction-detail').innerHTML = mandiri ? 'MANDIRI : 141-00-223344-55' : 'BCA : 0241772089';
}
...
function showUploadForm(){
    ...
    if(!nama || !tanggalValid){
        alert("Lengkapi data diri dan pastikan tanggal valid!");
        return;
    }
    document.getElementById('upload-section').classList.remove('d-none');
    document.getElementById('btn-db').classList.add('d-none');
}

// ==========================================
// 3. LOGIKA INTI: SIMPAN & TUNGGU ADMIN
// ==========================================

function simpanKeDB(){
    const nama = document.getElementById('custNama').value;
    const tgl = document.getElementById('custTanggal').value;
    const alm = document.getElementById('custAlamat').value;
    ...
    // STEP 1: Ubah tombol jadi "Sistem sedang memproses"
    document.getElementById('upload-section').classList.add('d-none');
    const btnWait = document.getElementById('wait-msg');
    btnWait.classList.remove('d-none');
    btnWait.innerHTML = `<i class="fas fa-spinner fa-spin me-2"></i> Sistem sedang memproses...`;
    ...
    formData.append('jenis_bayar', jenisBayar.value);
    formData.append('paket', document.getElementById('nama_paket').value);
    formData.append('total', document.getElementById('total_harga').value);
    ...
    .then(data => {
        if(data.success){
            lastOrderId = data.id;
            // STEP 2: Mulai pantau aksi dari halaman Admin
            mulaiCekKonfirmasiAdmin(data.id);
        } else {
...
function mulaiCekKonfirmasiAdmin(id){
    // Script ini akan terus bertanya ke server apakah Admin sudah klik 'Konfirmasi' di web admin
    const interval = setInterval(() => {
        fetch(`cek_status.php?id=${id}`)
        .then(res => res.json())
        .then(data => {
            // Jika admin sudah mengubah status di halaman admin menjadi 'dikonfirmasi'
            if(data.status === 'dikonfirmasi'){
                clearInterval(interval); // Berhenti mengecek

                // STEP 3: Ubah tombol jadi "Konfirmasi ke Owner"
                document.getElementById('wait-msg').classList.add('d-none');
                const btnWA = document.getElementById('btn-wa');
                btnWA.classList.remove('d-none');
                btnWA.innerHTML = `<i class="fab fa-whatsapp me-2"></i> Konfirmasi pesanan ke owner`;
            }
        })
        .catch(err => console.log("Menunggu Admin mengonfirmasi di web..."));
    }, 2000); // Cek setiap 2 detik agar terasa cepat (real-time)
}
...
// ==========================================
// 4. KIRIM WA (SETELAH ACC ADMIN)
// ==========================================
function kirimWA(){
    const nama = document.getElementById('custNama').value;
    const paket = document.getElementById('nama_paket').value;
    const total = document.getElementById('total_harga').value;
    const fRupiah = (angka) => "Rp" + parseInt(angka).toLocaleString('id-ID');

    const pesan = encodeURIComponent(
        `Halo Owner D-Mascha,\n\n*PESANAN TERKONFIRMASI ADMIN*\n----------------------------\nID : #${lastOrderId}\nNama : ${nama}\nPaket : ${paket}\nTotal : ${fRupiah(total)}\n----------------------------\nPesanan saya sudah dikonfirmasi admin di web. Mohon diproses ya Owner!`
    );

    window.open(`https://wa.me/6285178911709?text=${pesan}`, '_blank');
}
</script>
